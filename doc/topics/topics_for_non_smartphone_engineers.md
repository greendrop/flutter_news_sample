...
- スマホアプリ自体で即時または決められた時間に通知を表示ができる
...
- iOS: APNs(Apple Push Notification service)、Android: FCM(Firebase Cloud Messaging)で送信する
...
Web ページ内やメール文章内のリンクからスマホアプリを起動できる。

### カスタムスキーマ

- `myapp:://` のようにスマホアプリ独自のカスタムスキーマ設定ができる
- そのカスタムスキーマを指定したリンクを作成しそれをクリックすると、そのスマホアプリが起動する
...
- 1〜2 日程度
- 審査期間が長引くこともある
  - 初回提出: 初めて App Store にアプリを提出する場合、審査が厳しく行われるため、時間がかかることもある。
  - 大きなアップデート: アプリの機能が大幅に変更されるようなアップデートの場合も、審査に時間がかかることもある。
  - 審査項目への違反: ガイドラインに違反している部分があると、修正を求められ、審査が遅れる可能性もある。
  - 審査の混雑: 審査依頼が集中している時期は、審査に時間がかかることもある。
...
- 1〜2 日程度
- 審査期間が長引くこともある
  - アプリの複雑さ: 機能が多いアプリや、新しい技術を採用しているアプリは、審査に時間がかかることもある。
  - 審査状況: 審査依頼が集中している時期や、重大なバグが見つかった場合などは、審査が遅れることもある。
